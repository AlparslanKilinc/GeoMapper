      ...
      <Toolbar className={'navigationBar'}>
        ...
        <div className={'iconContainer'}>
          {isExplorePage && <Search/>}
            <IconButton id = "exploreButton" color="inherit" aria-label="explore" onClick = {navigateToExplorePage}>
              <ExploreIcon />
            </IconButton>
            <IconButton id = 'mapCreation' color="inherit" aria-label="add" onClick = {navigateToMapCreation}>
              <AddIcon />
            </IconButton>
          <AuthButton loggedIn={loggedIn} openConfirmationModal={openConfirmationModal} setPath = {setPath} />
        </div>
      </Toolbar>
      <Modal open={isModalOpen} onClose={closeConfirmationModal}>
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 300, pb: 10, bgcolor: 'background.paper', boxShadow: 24, p: 4 }}>
